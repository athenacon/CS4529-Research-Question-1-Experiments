...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, planes, num_caps, caps_size, depth, cfg_data, mode):
        ...
        # CIFAR-10 has 3 channels (RGB) and 10 classes
        ...
        final_shape = 7 if depth == 1 else 4
        # EM routing initialisation of the capsule network
        if self.mode == 'EM':
            # Since we have a ResNet-50, we have 2048 channels in the final layer so we need to change the number of channels in the convolutional layers
            # to enable the correct number of capsules to be created
            
            # Primary Capsule Layer
            self.conv_a = nn.Conv2d(2048, num_caps, kernel_size=3, stride=1, padding=1, bias=False)
            self.conv_pose = nn.Conv2d(2048, num_caps*caps_size, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        if self.mode == 'EM': 
            ...
            # Pass the output of the primary capsule layer through the Class Capsule Layer
            a, _ = self.fc(a, pose)
            # Then flatten and log the output to get the final output, the final image classifications
